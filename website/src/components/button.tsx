...
export function LinkButton({
  ...
  if (external) {
    ...
    // or `target` can't strip the security defaults. className stays
    // last because it's already merged with the caller's className via
    // classes() — replacing it via {...rest} would also discard the
    // variant styles, so we deliberately let our composed value win.
    return (
      <a
        href={href}
        {...rest}
