...
# so the "Enable Semantic Search" button in Astrolabe settings works.
#
# Runs as a before-starting hook (idempotent — skips if already configured).
set -e
...
MCP_PUBLIC_URL="${MCP_SERVER_PUBLIC_URL:-http://localhost:8000}"
...
# All scopes the MCP server supports
