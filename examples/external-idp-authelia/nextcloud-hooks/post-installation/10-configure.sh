...
# Authelia matches) from this. It must be the URL your BROWSER uses.
...
# Authelia uses the self-signed CA in ../certs (it will not speak OIDC over
# http). Nextcloud must accept it. With a publicly-trusted certificate — i.e.
# any real deployment — drop both of these.
occ config:system:set httpclient.allowselfsigned --value=true --type=boolean
...
# The MCP server calls back into Nextcloud with the user's Authelia token, so
...
# Authelia expects the client secret in the request body.
occ config:system:set user_oidc default_token_endpoint_auth_method --value='client_secret_post'

echo "==> Registering Authelia as the provider"
...
echo "==> Done. Log in at $NC_PUBLIC_URL as alice / password (via Authelia)."
